...
if ( ! class_exists( 'RascalsThemePanel_css' ) ) {

	class RascalsThemePanel_css extends RascalsThemePanel {
        ...
        */
		public function render() {
			...
			if ( isset( self::$_option['dependency']) && is_array( self::$_option['dependency'] ) ) {
				echo '<div class="box-row multi-options css-options clearfix dependent-hidden ' . self::$_option['classes'] . '" data-depedency-el="' . esc_attr( self::$_option['dependency']['element'] ) .'" data-depedency-val="'.esc_attr( implode(',', self::$_option['dependency']['value'] ) ).'" data-id="' . esc_attr( self::$_option['id'] ) . '">';
			} else {
				echo '<div class="box-row multi-options css-options clearfix ' . self::$_option['classes'] . '">';
			}
					...
					echo '<div class="box-tc box-tc-input">';
						...
						if ( isset( self::$_option['options'] ) && is_array( self::$_option['options'] ) ) {
							...
							foreach ($options as $key => $default) {
								...
								echo '<p><label>' . esc_attr( $default['name'] ) . '</label><input type="text" value="' . esc_attr( $default['value'] ) . '"  data-multi-default="' . esc_attr( $default['value'] ) . '" class="multi-option text-input"/></p>';
							}
						...
						echo '<input name="' . esc_attr( self::$_option['id'] ) . '" id="' . esc_attr( self::$_option['id'] ) . '" value="' . esc_attr( self::$_option['std'] ) . '" class="multi-data"/>';
