	...
	<div class="<?php echo esc_attr( implode(' ', $container_classes ) ) ?>">
		...
		<?php while ( have_posts() ) : the_post(); ?>
		        ...
		        if ( function_exists('epron_show_ad') ) {
		            echo epron_show_ad( 'article_top' );
		        }
		    	?>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class( 'single-post' ); ?>>
					...
					if (function_exists('epron_featured') ) {
						echo epron_featured( get_the_ID() );
					}
			    	...
			    	// Render content via Page Builder 
			  		epron_get_content();
		       		...
		       		wp_link_pages( array(
			            'before'    => '<div class="page-links">' . esc_html__( 'Jump to Page', 'epron' ),
			            'after'     => '</div>',
			        ) );
			        ...
			        // AD Spot
			        if ( function_exists('epron_show_ad') ) {
			            echo epron_show_ad( 'article_bottom' );
			        }
